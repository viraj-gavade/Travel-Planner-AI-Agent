...
@tool
def budget_estimation_tool(input: dict) -> dict:
    """
    ...
    Args:
        input: Dictionary with flight_cost, hotel_cost_per_night, number_of_days (all required), daily_expenses (optional)
    ...
    flight_cost = _parse_int(input.get("flight_cost", 0)) if isinstance(input, dict) else 0
    hotel_cost_per_night = _parse_int(input.get("hotel_cost_per_night", 0)) if isinstance(input, dict) else 0
    number_of_days = _parse_int(input.get("number_of_days", 0)) if isinstance(input, dict) else 0
    daily_expenses = _parse_int(input.get("daily_expenses", 1000), 1000) if isinstance(input, dict) else 1000
